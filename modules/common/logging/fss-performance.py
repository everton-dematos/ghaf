...
def main():
    ...
    if any(not re.fullmatch(r"[a-zA-Z0-9_-]+\.service", unit) for unit in args.units):
        parser.error("only plain service names are accepted")
    boot = Path("/proc/sys/kernel/random/boot_id").read_text().strip()
    directory = Path(
        tempfile.mkdtemp(prefix=f"fss-performance-{node}-{boot}-", dir="/tmp")
    )
    ...
    with ExitStack() as stack:
        ...
        while boot_seconds() < 1800:
            ...
            for unit in args.units:
                ...
                files[unit].flush()
            now = boot_seconds()
            target = min(1800, (math.floor(now / 10) + 1) * 10)
