...
let
  ...
  storageOptions = bytes: {
    maxStateBytes = mkOption {
      ...
      default = bytes;
      description = "Maximum evidence file bytes, including pending sidecars. Capacity exhaustion stops ingestion without deleting evidence.";
    };
    maxStateEntries = mkOption {
      type = types.ints.positive;
      default = 100000;
      description = "Maximum evidence entries and replay-index size. No automatic pruning.";
    };
  };
  ...
  options.ghaf.logging.logseald = {
    ...
    sealer = storageOptions (1024 * 1024 * 1024) // {
      ...
      maxChainBytes = mkOption {
        ...
        description = "Maximum central ledger bytes attributable to a single producer identity.";
      };
      maxChainEntries = mkOption {
        type = types.ints.positive;
        default = 20000;
        description = "Maximum central ledger entries attributable to a single producer identity.";
      };
