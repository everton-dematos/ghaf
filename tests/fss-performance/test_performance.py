...
SCRIPT = (
    Path(__file__).resolve().parents[2] / "modules/common/logging/fss-performance.py"
)
...
class PerformanceTests(unittest.TestCase):
    ...
    def test_memory_and_main_process(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            group = root / "cgroup/system.slice/journal-fss-setup.service"
            ...
            (process / "cgroup").write_text(
                "0::/system.slice/journal-fss-setup.service\n"
            )
            properties = {
                "ControlGroup": "/system.slice/journal-fss-setup.service",
                "MainPID": "123",
